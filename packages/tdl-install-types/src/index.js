...
async function fromPrebuiltTdlib () {
  try {
    ...
    // If not existent (e.g. because of an older version of prebuilt-tdlib),
    // fallback to the prebuilds/tdlib.json file
    const tdlibJsonFile = path.join(prebuiltTdlib, 'prebuilds', 'tdlib.json')
    const tdlibJson = JSON.parse(fs.readFileSync(tdlibJsonFile).toString())
    if (typeof tdlibJson?.version === 'string') {
      if (typeof tdlibJson?.commit === 'string') {
        await fromGitRef(tdlibJson.commit, tdlibJson.version)
        return
      }
      // If the 'commit' field does not exist, fallback to 'ref'
      if (typeof tdlibJson?.ref === 'string') {
        await fromGitRef(tdlibJson.commit, tdlibJson.version)
        return
      }
    }

    // If nothing works, fallback to executing getTdjson() and extracting tdlib
