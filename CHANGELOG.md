...
- Better detection of `prebuilt-tdlib`: can now use TDLib commit and version
  from `prebuilt-tdlib/package.json` or `prebuilt-tdlib/prebuilds/tdlib.json`
  instead of always extracting the commit hash from the library file. The
  `--prebuilt-tdlib` option is replaced with the `prebuilt-tdlib` argument
  (`--prebuilt-tdlib` is deprecated).
-->
